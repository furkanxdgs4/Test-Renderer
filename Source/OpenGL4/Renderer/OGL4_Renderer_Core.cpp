...
namespace OpenGL4 {
	...
	void OpenGL4_Renderer::Set_DepthTest(GFX_API::DEPTH_MODEs MODE, GFX_API::DEPTH_TESTs TEST) {
		...
		case GFX_API::DEPTH_MODEs::DEPTH_READ_ONLY:
			...
			return;
		default:
			TuranAPI::LOG_ERROR("Intended Depth Buffer mode isn't supported in OGL4_Renderer::Set_DepthTest()!");
			return;
		}
	}
	void OpenGL4_Renderer::Set_CullingMode(GFX_API::CULL_MODE MODE) {
		...
		case GFX_API::CULL_MODE::CULL_BACK:
			...
			return;
		default:
			TuranAPI::LOG_ERROR("Intended Culling Mode isn't supported by Set_CullingMode!");
			return;
		}
	}
	...
	void Bind_Uniform(const unsigned int& PROGRAM_ID, const GFX_API::Material_Uniform* uniform) {
		...
		if (location == -1) {
			//TuranAPI::LOG_WARNING("Intended uniform: " + uniform->VARIABLE_NAME + " can't be found by Bind_Uniform()!");
			return;
		}
		if (uniform->DATA == nullptr) {
			//TuranAPI::LOG_WARNING("Uniform: " + uniform->VARIABLE_NAME + " data isn't set!");
			return;
		}
		...
		case GFX_API::DATA_TYPE::VAR_MAT4x4:
			...
			break;
		default:
			TuranAPI::LOG_ERROR("Error: Sending an unsupported uniform type! Nothing happened!");
			return;
		}
	}
