...
namespace TuranEditor {
	...
	void* FirstCompute::RAYTRACECAMERABUFFER_DATA = new bool[80]{};
	...
	FirstCompute::FirstCompute() {
		...
		{
			RAYTRACECAMERABUFFER_ID = GFXContentManager->Create_GlobalBuffer("RAYTRACE_CAMERA", RAYTRACECAMERABUFFER_DATA, 80, GFX_API::BUFFER_VISIBILITY::CPUREADWRITE_GPUREADWRITE);
		}
		...
		//OUTPUT IMAGE CREATION
		{
			...
			TEXTURE->DATA = nullptr; TEXTURE->DATA_SIZE = 512 * 512 * 16;
			TEXTURE->Has_Mipmaps = false; TEXTURE->WIDTH = 512; TEXTURE->HEIGHT = 512;
	...
	void FirstCompute::Execute() {
		...
		{
			mat4 view_mat;
			view_mat = inverse(lookAt(RAYTRACECAMERA_POS, RAYTRACECAMERA_FRONTVECTOR + RAYTRACECAMERA_POS, vec3(0, 1, 0)));
			memcpy(RAYTRACECAMERABUFFER_DATA, &RAYTRACECAMERA_POS, 12);
			memcpy(((bool*)RAYTRACECAMERABUFFER_DATA) + 16, &view_mat, 64);
			GFXContentManager->Upload_GlobalBuffer(RAYTRACECAMERABUFFER_ID);
		}
		GFXRENDERER->Compute_Dispatch(ComputeShaders[0], vec3(512, 512, 1));
	}
}